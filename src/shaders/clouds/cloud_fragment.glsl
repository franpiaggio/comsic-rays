...
const vec3 skycolour1 = vec3(0.26, 0.5, .3);
const vec3 skycolour2 = vec3(0.);
...
void main()	{
    ...
    skycolour = mix(skycolour, vec3(.8, .6, .2), p.x);
    skycolour = mix(skycolour, vec3(.1, .3, .1), p.x/p.y);

    vec3 cloudcolour = vec3(2.1, 1.1, 1.2) * clamp((clouddark + cloudlight*c), 0.0, 1.0);
    // vec3 skycolour = mix(skycolour2, skycolour1, p.y);
